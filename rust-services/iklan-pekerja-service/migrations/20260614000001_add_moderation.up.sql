-- Add moderation_status + deleted_at (soft-delete) + foto_urls for iklan_pekerja.
...
CREATE INDEX IF NOT EXISTS idx_pekerja_moderation
    ON iklan_pekerja.iklan (moderation_status)
    WHERE deleted_at IS NULL;

-- Replace the old is_active-only index with a combined one for public listing.
DROP INDEX IF EXISTS iklan_pekerja.idx_pekerja_created;
CREATE INDEX idx_pekerja_created_active
    ON iklan_pekerja.iklan (created_at DESC)
