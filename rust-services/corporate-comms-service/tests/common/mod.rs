...
pub use fixtures::{clean_test_data, seed_admin, seed_article, seed_user};

/// Test app — build router untuk corporate-comms-service saja dengan pool test.
/// Router ditempatkan di bawah `/api/v1/articles` agar path cocok dengan main.rs.
pub async fn build_test_app(pool: PgPool) -> Router {
    ...
    let comms_router = corporate_comms_service::router(pool, auth_client, None, None, None);

    Router::new().nest("/api/v1/articles", comms_router)
}
