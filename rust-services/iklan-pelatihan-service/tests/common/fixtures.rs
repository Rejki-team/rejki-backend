...
#[allow(dead_code)]
pub async fn seed_iklan_pelatihan(
    pool: &PgPool,
    poster_id: Uuid,
    suffix: &str,
    created_by_role: &str,
    ...
    let status = if status_lifecycle == "default" {
    ...
    sqlx::query(
        "INSERT INTO iklan_pelatihan.iklan
            (id, poster_id, judul, penyelenggara, deskripsi, is_active,
             moderation_status, status, created_by_role, created_at, updated_at)
         VALUES ($1, $2, $3, $4, $5, true, $6, $7, $8, now(), now())
         ON CONFLICT (id) DO NOTHING",
    )
    .bind(id)
    .bind(poster_id)
    .bind(format!("Pelatihan {suffix}"))
    .bind(format!("Penyelenggara {suffix}"))
    .bind(format!("Deskripsi pelatihan {suffix}"))
    .bind("active")
    .bind(status)
    .bind(created_by_role)
    .execute(pool)
    .await
    .expect("seed_iklan_pelatihan: insert failed");

    id
}

pub async fn clean_test_data(pool: &PgPool) {
    sqlx::query("DELETE FROM iklan_pelatihan.pelatihan_badge WHERE user_id IN (SELECT id FROM auth.users WHERE email LIKE '%@test.rejki.internal')")
        .execute(pool).await.ok();
    sqlx::query("DELETE FROM iklan_pelatihan.pelatihan_enrollment WHERE user_id IN (SELECT id FROM auth.users WHERE email LIKE '%@test.rejki.internal')")
        .execute(pool).await.ok();
    sqlx::query("DELETE FROM iklan_pelatihan.iklan_suspension WHERE iklan_id IN (SELECT id FROM iklan_pelatihan.iklan WHERE poster_id IN (SELECT id FROM auth.users WHERE email LIKE '%@test.rejki.internal'))")
        .execute(pool).await.ok();
    sqlx::query("DELETE FROM iklan_pelatihan.iklan WHERE poster_id IN (SELECT id FROM auth.users WHERE email LIKE '%@test.rejki.internal')")
        .execute(pool).await.ok();
    sqlx::query("DELETE FROM user_svc.profiles WHERE auth_id IN (SELECT id FROM auth.users WHERE email LIKE '%@test.rejki.internal')")
        .execute(pool).await.ok();
