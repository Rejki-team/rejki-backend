...
pub async fn clean_test_data(pool: &PgPool) {
    // Hapus messages milik user test
    sqlx::query(
        "DELETE FROM chat.messages WHERE sender_id IN \
         (SELECT id FROM auth.users WHERE email LIKE '%@test.rejki.internal')",
    )
    .execute(pool)
    .await
    .expect("clean_test_data: messages failed");

    // Hapus conversations milik user test
    sqlx::query(
        "DELETE FROM chat.conversations WHERE user_a IN \
         (SELECT id FROM auth.users WHERE email LIKE '%@test.rejki.internal') \
         OR user_b IN \
         (SELECT id FROM auth.users WHERE email LIKE '%@test.rejki.internal')",
    )
    .execute(pool)
    .await
    .expect("clean_test_data: conversations failed");
