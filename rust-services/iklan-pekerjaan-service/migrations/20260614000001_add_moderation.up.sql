-- Add moderation_status + deleted_at (soft-delete) + foto_urls for iklan_pekerjaan.
...
CREATE INDEX IF NOT EXISTS idx_pekerjaan_moderation
    ON iklan_pekerjaan.iklan (moderation_status)
    WHERE deleted_at IS NULL;

-- Replace the old is_active-only index with a combined one for public listing.
DROP INDEX IF EXISTS iklan_pekerjaan.idx_pekerjaan_created;
CREATE INDEX idx_pekerjaan_created_active
    ON iklan_pekerjaan.iklan (created_at DESC)
