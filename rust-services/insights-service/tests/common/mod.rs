...
pub use fixtures::{clean_test_data, executive_token, super_admin_token, user_token};

/// Test app — build router untuk insights-service saja dengan pool test.
pub async fn build_test_app(pool: sqlx::PgPool) -> Router {
    ...
    let insights_router = insights_service::router(pool, auth_client);

    Router::new().nest("/insights", insights_router)
}
...
/// Baca response body sebagai JSON.
