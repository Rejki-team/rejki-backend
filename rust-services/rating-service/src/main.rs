// Binary standalone rating-service. Hanya membutuhkan database; validasi lamaran
// (IklanPekerjaanClient) tidak tersedia — create_rating akan gagal dengan "layanan
// validasi lamaran tidak tersedia" (degradasi eksplisit, bukan silent 500).
#[tokio::main]
async fn main() {
    ...
    // Standalone: auth dummy (selalu invalid token — tidak ada validasi token nyata
    // tanpa auth-service berjalan; pola sama `report-service`/`insights-service`).
    ...
    let port = std::env::var("RATING_PORT")
        .ok()
    ...
    let listener = tokio::net::TcpListener::bind(format!("0.0.0.0:{port}"))
        .await
        .unwrap();
    tracing::info!("rating-service (standalone) listening on :{port}");
    axum::serve(listener, rating_service::router(pool, auth_client, None))
        .await
        .unwrap();
}
